...
description: "Improve DX and reliability in your MDX + TypeScript setup using two small but powerful configuration changes."
...
![Path aliases do not work in MDX files out of the box — leading to an error when importing components](error-message.png)

To enable this behavior, you need to opt into JavaScript file support by enabling `allowJs` in your `tsconfig.json`:

```json filename="tsconfig.json" add={7}
{
  "compilerOptions": {
    "baseUrl": ".",
    "paths": {
...
![Without type checking enabled, props can be passed incorrectly to components in MDX without any warnings or errors](invalid-props-without-typechecking.png)
...
```json filename="tsconfig.json" highlight={2-4}
{
  "mdx": {
    "checkMdx": true
  }
...
![With type checking enabled, invalid props in MDX trigger a clear error message](invalid-props-with-typechecking.png)
